# dashboard-vulnerabilities-view Specification

## Purpose

Gives the dashboard a dedicated `/vulnerabilities` page so users can browse the CVE catalog directly and pivot from a CVE to the nodes it affects, instead of only seeing CVEs buried per-node in the explorer's node detail drawer. The page is reachable from the persistent dashboard navigation and from the command palette, and reuses the existing design-system tokens, theming, table/pagination patterns, and the `Pill` `kind="CVE"` component.
