# nostr-cdn-recon Specification

## Purpose

Measures how centralized the Nostr relay network is by classifying each relay host against the published CIDR ranges of major CDNs (Cloudflare, CloudFront, Fastly). This is a pure DNS + CIDR-matching reconnaissance phase (phase 0) that consumes no Shodan credits: it normalizes relay host inputs, resolves their A/AAAA records, assigns a per-host verdict, and writes a JSON dump that is later imported into the database. Origin unmasking (phase 2) is out of scope.
