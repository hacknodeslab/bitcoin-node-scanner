...
- [x] 8.2 Grammar end-to-end: `risk` / `country` / `exposed` / `tor` (via `=`, the v0 grammar already shipped in §5.6 — `:` in the spec text is shorthand). Backend (8.2a): `/api/v1/nodes` accepts `exposed` and `tor` query params; `tor=true` matches `tags LIKE %tor% OR hostname LIKE %.onion`, `tor=false` returns 400 (NULL-aware negation deferred until a dedicated `is_tor` column lands; the query bar never emits it). Frontend (8.2b): `lib/query-grammar.ts` (`tokensToFilters` / `parseQueryToFilters`) maps parsed tokens to `NodeListParams` with diagnostics; `Explorer` (client) owns the applied query and lifts filters into `NodeTable`; `QueryBarController` is the controlled input — Enter applies, render-time sync from props handles external resets. Active-filters preview only renders for the *applied* value (orange chevron is the active-filter signal). Suite: 80 frontend + 30 backend.
...
- [x] 8.5 `ExplorerFooter` becomes a client component cabling `useScanJob`. Right-anchored secondary `Button` ("run scan" → "scanning…" while pending/running, disabled). Inline status with state-coloured `dot` glyph (queued/scanning…/done/failed). On terminal status, mutates `/api/v1/stats` and every `/api/v1/nodes*` SWR key so the strip + table refresh without a manual reload. `start()` errors surface inline (no crash) — useful when SHODAN_API_KEY is missing. Prop overrides (`job`, `onStart`, `busyOverride`) keep tests deterministic. 6 vitest cases covering idle / running / completed / failed / click / error-surface.
